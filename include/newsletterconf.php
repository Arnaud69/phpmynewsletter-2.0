<?php
echo '<header><h4>'.tr("NEWSLETTER_SETTINGS").', <a href="#codeHtml">'.tr("CODE_TITLE").'</a></h4></header>';
echo '<div class="row">';
        ...
        if(empty($action)) {
            ...
            if(isset($list_id)&&!empty($list_id)) {
                ...
                <div class='form-group'><fieldset>
                    ...
                    <textarea cols='70%' rows='8' class='form-control'><form action='/".$row_config_globale['path']."subscription.php' method='post' target='pmnlwindow' onsubmit=\"window.open('/".$row_config_globale['path']."subscription.php', 'pmnlwindow', 'scrollbars=yes,width=700,height=210');return true\">
                ...
                <fieldset>
                    ...
                    <textarea cols='70%' rows='8' class='form-control'><form action='/".$row_config_globale['path']."subscription.php' method='post' target='_blank'>
                    <input type='text' name='email_addr' value='' size='30'>
                    <input type='hidden' name='list_id' value='$list_id'>
                    <input type='hidden' name='op' value='join'>
                    <input type='submit' value='".tr("SUSCRIBE")."'>
            ...
            echo "</div>";
            echo "<script src='".($row_config_globale['path']=='/'?$row_config_globale['path']:'/'.$row_config_globale['path'])."js/tinymce/tinymce.min.js'></script>";
            echo "
            <script>
            tinymce.init({
