<?php
    ...
    if($row_cnt['CPTID'] > 0){
            ...
            AmCharts.makeChart("chartdiv",
                {
                    ...
                    "dataProvider":[<?php
                        foreach($array_stats as $row){
                            echo '{"date":"'.$row['dt'].'","c1":'.($row['cpt']!=''?$row['cpt']:0).',"c2":'.($row['error']!=''?$row['error']:0).',"c3":'.($row['TID']!=''?$row['TID']:0).',"c4":'.($row['TOPEN']!=''?$row['TOPEN']:0).',"c5":'.($row['leave']!=''?$row['leave']:0).',"c6":'.($row['CPT_CLICKED']!=''?$row['CPT_CLICKED']:0).',"cp_id":'.$row['id_mail'].'},';
                        }
                        ?>],"export":{"enabled": true}});
        </script>
        <?php
        ...
        'SELECT platform,COALESCE(COUNT(*),0) AS data
            FROM ' . $row_config_globale['table_tracking'] . ' 
                WHERE platform!=\'\' 
    ...
    <table class="table table-striped">
        ...
        <tr>
            <td width="30%"><div align="center"><h4><?php echo tr("CLICKED_BY_DOMAINS"); ?></h4></div><canvas id="PmnlCntClkDomain" /></td>
            <td width="30%"><div align="center"><h4><?php echo tr("CLICKED_DISTINCT_DOMAINS"); ?></h4></div><canvas id="PmnlDistctDomain" /></td>
            <td width="30%"><div align="center"><h4><?php echo tr("SUPPORT"); ?></h4></div><canvas id="PmnlStatsDevicetype" /></td>
        </tr>
        <tr>
        ...
        <tr>
            <td colspan=3><h4><?php echo tr('CLICK_BY_HOURS'); ?></h4><canvas id="ClicByHours" style="width:70%;height:300px;"></canvas> </td>
        </tr>
        </table>
        <script>
                ...
                case 'ctry' :
                    $sql = 'SELECT DISTINCT(t.country),
                            ...
                            FROM ' . $row_config_globale['table_tracking'] . ' t
                                LEFT JOIN test_codes c ON t.country=c.country
                            WHERE t.city!="" 
